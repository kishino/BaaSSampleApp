 ...
 * @constructor
 */
angular.module('myApp').controller('MainController', function MainController($q) {
  var TENANT_KEY = 'ここにテナントキー';
  var USER_ACCESS_TOKEN_KEY = 'userAccessToken';
  var SENDER_ID = '「Androidの設定」で参照したSenderID';
  ...
  function registerUser() {
    ...
    if (!userAccessToken) {
      ...
      RKZClient.registUser(userData, function (userData) {
        ons.notification.toast('ユーザー登録しました。', { timeout: 3000 });
        console.info(userData.user_access_token + "," + userData.user_no);
